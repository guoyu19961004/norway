...
#            link_dump_path = link_dump.name
            ...
            self.dump_page_ingentia(visit_link, config['InputEncoding'], link_dump_path, config['UserAgent'])
            # link_dump.close()
            raw_urls = self.get_all_urls_from_dump(link_dump_path)

            # print  "raw_urls"
            # pprint.pprint(raw_urls)
            # print

            fixed_urls = self.fix_relative_urls(config['BaseUrl'], raw_urls)
            ...
            if self.match_filter(link, self.convert_to_common_regular(config['SubsourceUriPattern'])):
                ...
                if valid:
                    ...
                    i += 1
                    # shutil.copyfile(link_dump_path, os.path.join(ts.TMONKEY_DIR, "libs", "%d.html" %(i)))
                    self.failed_subsource[link] = None
                    self.print_detail(msg)

        # pprint.pprint(self.subsources)
        # print
    ...
    def parse_config(self, config_string):
        try:
            ...
            config['BaseUrl'] = root.find('.//BaseUrl').text if root.find('.//BaseUrl') is not None else ''
            config['ErrorNamePattern'] = root.find('.//ErrorNamePattern').text if root.find('.//ErrorNamePattern') is not None else ''
    ...
    def get_all_urls_from_dump(self, dump_path):
        ...
        try:
            ...
            for raw_url in raw_urls:
                ...
                # print config['ErrorNamePattern']
                # print self.convert_to_common_regular(config['ErrorNamePattern'])
                # print
                if not re.search("^Sub-Forums: \d+$", title):
...
#                               stdout=subprocess.PIPE).stdout.read()
    ...
    def dump_page(self, link, encoding, fp , user_agent=None):
        ...
        try:
            print "into dump_page"
            http_dump_path = os.path.join(ts.TMONKEY_DIR, "libs", "httpdump")
            req = urllib2.Request(link, headers={ 'User-Agent' : user_agent })
            response = urllib2.urlopen(req, timeout=20)
            print "response"
            content = response.read()
            http_dump = open(http_dump_path, "w")
            if re.search("&#\d+;", content):
                print "html code"
                http_dump.write(re.sub('&#\d+;', '', content))
            else:
                print "normal"
                http_dump.write(content)
            http_dump.close()
            subprocess.call('java -jar {0} --encoding={1} "file:///{2}"'.format(ts.CLEAN_TAGSOUP_JAR_PATH,
                                                                            encoding,
                                                                            http_dump_path),shell=True,
                            stderr=subprocess.PIPE,
                            stdout=fp)
        except urllib2.URLError:
            self.print_detail('Request timed out, failed to dump link: {0}'.format(link))
        except:
    ...
    def dump_page_ingentia(self, link, encoding, fp , user_agent=None):
        ...
        try:
            ...
            pass
        ingentia_path = os.path.join(ingentia_root_path, "ingentia-test-crawler-3.0.1-SNAPSHOT-jar-with-dependencies.jar")
        print link
        log = subprocess.Popen('java -jar %s -c clean tagsoup "%s" "%s"' % ("ingentia-test-crawler-3.0.1-SNAPSHOT-jar-with-dependencies.jar", link, encoding),
                        shell=True,
                        cwd = ingentia_root_path,
                        stdout=subprocess.PIPE).stdout.read()
        shutil.move(cleanedump, fp)
