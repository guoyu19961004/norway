...
(: Global variables :)
declare variable $validateUri := "bbs.zol.com.cn";
...
declare function t:getLanguage() as node() {
        ...
        <nonNumeric value =" 1 " >一</nonNumeric>
        <nonNumeric value =" 2 " >二</nonNumeric>
        <nonNumeric value =" 3 " >三</nonNumeric>
        <nonNumeric value =" 4 " >四</nonNumeric>
        <nonNumeric value =" 5 " >五</nonNumeric>
        <nonNumeric value =" 6 " >六</nonNumeric>
        <nonNumeric value =" 7 " >七</nonNumeric>
        <nonNumeric value =" 8 " >八</nonNumeric>
        <nonNumeric value =" 9 " >九</nonNumeric>
        <nonNumeric value =" 10 ">十</nonNumeric>
    </language>
};
...
{
    let $posts := $rootNode//h:a[@class="user-name"]//ancestor::h:table[matches(@class,"post-list")][1]
    let $posts := if(empty($posts))
                then $rootNode//h:span[@class="user-name"]//ancestor::h:table[matches(@class,"post-list")][1]
                else $posts
    let $posts := for $post in $posts 
                where empty($post//h:span[@class="praise-comments"])
                return $post 
...
{
    ig:extractEx($documentUri,"d\d+_(\d+)")
};

(:7 - Fetch post id:)
declare function t:getPostId($post as node()*, $page as xs:string, $index as xs:string) as xs:string
{
    let $threadId := ig:extractEx($documentUri,"d\d+_(\d+)")
   (: let $postNum := if(exists($post//preceding::h:a[matches(@id,"reply\d+")]))
    then ig:extractEx(($post//preceding::h:a[matches(@id,"reply\d+")])[1]/@id,"reply(\d+)")
    else "0":)
   (: let $postNum := for $a in $post return
                if(exists($a//h:span[@class="floor"]))
                then ig:extract(($a//h:span[@class="floor"])[1]/text(),"\d+")
                else "0":)
   (: let $postNum := if($page="1" and $index="1")
                then "0"
                else ig:extract(($post//h:span[@class="floor"])[1]/text(),"\d+")
    let $postId := concat($threadId,"_",$postNum):)
    let $postId := if(matches($post/@class,"replyList"))
                then ($post//h:div[matches(@data-replyid,"^\d+$")])[1]/@data-replyid
                else "0"
    let $postId := if(string-length($postId)=0)
                then ($post//following::h:div[matches(@data-replyid,"^\d+$")])[1]/@data-replyid
                else $postId
    return $postId
};
...
{
    let $authorNodes := ($post//h:a[@class="user-name"])[1]
    let  $authorNodes := if(empty($authorNodes))
                then ($post//h:span[@class="user-name"])[1]
                else $authorNodes
    return $authorNodes
};
...
{
    let $postNum := if(matches($post/@class,"replyList"))
                then ig:extract(($post//h:span[@class="floor"])[1]/text(),"\d+")
                else "0"
    let $postUrl := if($postId="0")
                then replace(ig:removeParamValueFromQuery($documentUri,"replies"),"(\d+).html","1.html")
                else concat(ig:removeParamValueFromQuery($documentUri,"replies"),"#reply",$postNum)
    return $postUrl
};
...
{
    ig:nsj($rootNode//h:div[@id="bookTitle"]/h:h1/text())
};
...
{
    let $currentPage := if(matches($documentUri,"d\d+_\d+_\d+.html"))
            then ig:extractEx($documentUri,"(\d+).html")
            else "1"
...
{
    let $time := ig:nsj(($post//h:span[@class="publish-time"])[1]/text())
    let $time := ig:extract($time,"\d{4}-\d{2}-\d{2}\s+\d+:\d+:\d+")
    return $time
};
...
{
   let $message := ($post//h:div[@id="bookContent"])[1]
   let $message := if(exists($message))
                   then $message
                   else ($post//h:td[@class="post-main"])[1]
   let $message := ig:removeTagByName($message,"br")
   return $message
};
