...
(:uri is the target to solve, baseUri is the Base Uri to resolve uri with, concatUri is the subUri to concat it to uri.
if concatUri is '#SAME#', it's the same as baseUri! :)
declare function ig:resolveUri($uri as xs:string, $baseUri as xs:string, $concatUri as xs:string) as xs:string
{
    if((string-length($uri) = 0) or (matches(lower-case($uri), '^none\.?$') or matches(lower-case($uri), '^deleted?none\.?$')))
    then ''
    else if(matches($uri, '^https?://.+'))
    then $uri
    else
        ...
        return
            if(matches($uri, '\\'))
            then
                let $concatUri := if($concatUri = '#SAME#') then $baseUri else $concatUri
                return concat($concatUri, $uri)
...
declare function t:translateRelativeTime( $timestamp as xs:string, $timePattern as xs:string, $lang as node() ) as xs:string
{
    ...
    let $durationOfYM := string-join($durationYM,"")
    let $durationOfYM := if (string-length($durationYM) > 0)
                             then concat("P",replace($durationOfYM,"P",""))
                             else ""
    let $durationOfDHMS := concat($durationD,string-join($durationHMS,""))
...
declare function d:generateCommentTime($blogInfo as node()*) as xs:string
{
    ...
    return if (matches($blogInfo/timestamp, "00-00-00")) then
        ...
        error(xs:QName('d:generateTime'), "Post time is incorrect if generate comment time.")
};
...
{
    let $t := tokenize($timestamp," ")
    let $timestamp := xs:dateTime( concat($t[3],"-",$t[1],"-",$t[2], "T",$t[4],":",$t[5],":",$t[6]))
...
declare function zf:parseTimestamp($rawTimestamp as xs:string, $timestampRegex as xs:string) as xs:string
{
    ...
    let $timestamp := if(string-length($rawTimestamp) = 0)then
...
declare function zf:blogpostTransientMeta() as node()
{
    let $ignorePage := xs:string(node-name(t:getPageType())) = "TRANSIENTMETA-ONLY"
...
declare function zf:commentTransientMeta($blogPost as node()*) as node() {
    ...
    <tMeta>  
        ...
        {
            if(not(empty(t:getVisitLinks())) and not(string-length(ig:nsj(t:getVisitLinks())) = 0)) 
                then
                    <entry>
                     <string>visit</string>
                         <list>
...
declare function zf:getTransientMeta($blogPost as node()*) as node()*
{
    let $commentTransientMeta  := zf:commentTransientMeta($blogPost)
    let $blogpostTransientMeta := zf:blogpostTransientMeta()
...
{
    let $blogPost := if($includeBlogPost)
                         then zf:getBlogPost()
                         else ()  
                              
    let $comments := if($includeComments)
                         then zf:getComments($blogPost)
                         else ()
                         
    let $tags     := if($includeBlogPost)
                         then zf:getTags()
                         else ()  
    return                    
        <no.integrasco.domain.xml.blog.BlogPost>
            ...
            <tags>{$tags}</tags>
            <meta/>
            <transientMeta>{zf:getTransientMeta($blogPost)}</transientMeta>
...
{
    let $postId             := replace( $documentUri, "https?://", "" )
...
(:META - START:)
(:1- May return <BLOGPOST-WITH-COMMENTS/>, <BLOGPOST/> <COMMENTS/>, <TRANSIENTMETA-ONLY/>,<GENERATEURL-ONLY/>
...
(:4 Use time to skip old blogPosts for the case that id changed, remember the pattern should be MM dd yyyy HH mm ss 
   please remember to let it empty if no use :) (: If you are not allowed to change the related global variables, please just let it empty :)
declare function t:validBlogPostTimePoint() as xs:string*
{
    "03 09 2012 00 00 00"
};

(:5 Use time to skip old comments for the case that id changed, remember the pattern should be MM dd yyyy HH mm ss 
   please remember to let it empty if no use :) (: If you are not allowed to change the related global variables, please just let it empty :)
...
{
    $rootNode//h:div[@class="entry"]//h:div[@class="post-meta"]
};
...
{
    let $timeNode:=$blogPost//h:div[@class="post-meta"]
    let $timeNode:=ig:removeTagByName($timeNode,"a")
    let $timeNode:=ig:nsj($timeNode)
    return $timeNode
};
...
{
    d:generateBlogPostId()
};

(:10 - Fetch the blog post message and refine it:)
declare function t:getBlogPostMessage($blogPost as node()*) as node()*
{
     let $messageNode := $blogPost
     let $exceptMessage:=$messageNode//h:div[@class="like-buttons clearfix"]
     let $exceptMessagefn:=insert-before( $exceptMessage/following-sibling::node(), 0, $exceptMessage )
    
     let $messageNode := $messageNode/node() except $exceptMessage
    
     let $messageNode :=ig:removeTagByAttribute($messageNode,"class","post-meta")
     let $messageNode :=ig:removeTagByName($messageNode,"h2")
...
{
    $rootNode//h:li[matches(@id,"dsq-comment-\d+")]
};
...
{
    $comment//h:cite[matches(@id,"dsq-cite-\d+")]
};
...
{
    d:generateCommentTime($blogInfo)       
};
...
{
   let $messageText:=$comment//h:div[@class="dsq-comment-message"]
   return $messageText
