...
{
    let $forumId := t:getForumId($rootNode)
    let $threadNodes :=  $rootNode//h:tr[matches(@id,concat("\d+_",$forumId,"_\d+"))]
    return
        $threadNodes
};
...
{
    let $threadLink := ig:nsj(($threadNode//h:a[matches(@href,"/\w+bbs/d\d+_\d+.html")])[1]/@href)
    let $threadLink := ig:resolveUri($threadLink,"http://bbs.zol.com.cn/")
    return
        $threadLink
};
...
{
    let $threadId := ig:extractEx($threadNode/@id,"\d+_\d+_(\d+)")
    return
        $threadId
};
...
{
   let $repliesNodes := $threadNode//h:td[@class = "reply"]//h:span
   let $repliesNodes := if (exists($repliesNodes))
                        then $repliesNodes
                        else $threadNode//h:div[@class = "sigle-reply"]/h:span[@class = "black"]
    return $repliesNodes
};
...
{
    let $threadUrl := replace($threadLink,".html",concat("_",$page,".html"))
   return
...
{
    let $date := ig:nsj($threadNode//h:td[@class="last-post"]/h:a[@class="date"]//text())
    let $date  := if($date="0000-00-00")
                   then "1970-01-01"
                   else $date
    return $date
};
...
{
    ig:extractEx($documentUri,"d(\d+)\.html")
};
...
{    
    let $a := ($rootNode//h:div[@class="page"])[last()]//h:a/@href
    let $pages := for $link in $a return xs:integer( if(ig:extractEx($link,"p(\d+).html"))
            then ig:extractEx($link,"p(\d+).html")
            else "1")
...
{
    let $currentPage := if(ig:extractEx($documentUri,"p(\d+).html"))
            then ig:extractEx($documentUri,"p(\d+).html")
            else "1"
...
{
    let $forumUrl := replace($documentUri,".html",concat("_p",$page,".html"))
